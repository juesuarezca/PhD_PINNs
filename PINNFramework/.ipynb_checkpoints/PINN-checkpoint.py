...
class PINN(nn.Module):
    ...
    def __init__(self, model: torch.nn.Module, input_dimension: int, output_dimension: int,
        ...
        if isinstance(initial_condition, InitialCondition):
            ...
            self.initial_condition_m = Loss_meas[0]
        else:
        ...
        joined_datasets_m = {"Initial_Condition": Loss_meas[0].dataset, "PDE": Loss_meas[2].dataset}
        if not self.is_hpm:
            if type(boundary_condition) is list:
                for bc in boundary_condition:
                    ...
                    self.boundary_condition_m = Loss_meas[1]
                    joined_datasets[bc.name] = bc.dataset
                    joined_datasets_m[bc.name] = Loss_meas[1][0].dataset
            else:
        ...
        self.dataset_m = JoinedDataset(joined_datasets_m)
    ...
    def pinn_loss_mse(self, training_data):
        """
        Function for calculating the PINN loss. The PINN Loss is a weighted sum of losses for initial and boundary
        condition and the residual of the PDE
        ...
        if type(training_data["PDE"]) is not list:
            pinn_loss = pinn_loss + self.pde_loss_m(training_data["PDE"][0].type(self.dtype), self.model)
        else:
            raise ValueError("Training Data for PDE data is a single tensor consists of residual points ")
        if not self.is_hpm:
    ...
    def fit(self, epochs, optimizer='Adam', learning_rate=1e-3, lbfgs_finetuning=True,
        ...
        if self.use_horovod:
            ...
            ####### Add second loss 
            train_sampler_m = torch.utils.data.distributed.DistributedSampler(
            self.dataset_m, num_replicas=hvd.size(), rank=hvd.rank())
            data_loader_m = DataLoader(self.dataset_m, batch_size=1,sampler=train_sampler_m)
