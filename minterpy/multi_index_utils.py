...
from minterpy.global_settings import INT_DTYPE, DEFAULT_LP_DEG, ARRAY
...
def list_insert_single(list_of_indices: List[ARRAY], index2insert: ARRAY) -> None:
    """ inserts a single index into a given list of indices maintaining lexicographical ordering

    the vector is not being inserted when it already exists in the list of indices
    """
    ...
    if nr_of_indices == 0:
        ...
        return
    first_entry = list_of_indices[0]
    if first_entry.shape != index2insert.shape:
        raise ValueError(f'shape mismatch: expected {first_entry.shape}, got {index2insert.shape}')
