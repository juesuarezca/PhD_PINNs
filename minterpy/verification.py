...
from minterpy.global_settings import DEFAULT_DOMAIN, FLOAT_DTYPE, INT_DTYPE, DEBUG
...
def rectify_eval_input(x, coefficients, exponents, verify_input):
    ...
    if len(coeff_shape) == 1:  # 1D: a single query polynomial
        ...
        if N != N_coeffs:
            raise ValueError(f'the coefficient amount {N_coeffs} does not match the amount of monomials {N}')
    ...
    if verify_input:
        ...
        for name, (value, exp_type) in expected_types.items():
            if value.dtype != exp_type:
                raise TypeError(f'expected dtype {exp_type} for {name} but the dtype is {value.dtype}')
...
def check_type(a, expected_type=np.ndarray, *args, **kwargs):
    if not issubclass(type(a), expected_type):
        raise TypeError(f'input must be given as {expected_type} (encountered {type(a)})')


def check_dtype(a: np.ndarray, expected_dtype):
    if a.dtype != expected_dtype:
        raise TypeError(f'input must be given as {expected_dtype} (encountered {a.dtype})')
...
def check_shape(a: np.ndarray, shape: Union[list, tuple] = None, dimensionality: int = None):
    """
    ...
    if a.ndim != dimensionality:
        raise ValueError(f'expected {dimensionality}D array, but encountered array of dimensionality {len(a.shape)}')
    if shape is None:
        return
    for dim, (true_size, expected_size) in enumerate(zip(a.shape, shape)):
        if isinstance(expected_size, int) and true_size != expected_size:
            raise ValueError(f'expected array of size {expected_size} in dimension {dim},'
...
DOMAIN_WARN_MSG = 'this may lead to unexpected behaviour, ' \
