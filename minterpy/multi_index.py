...
from minterpy.multi_index_utils import _expand_dim, \
    ...
    make_complete, verify_lp_deg, get_exponent_matrix
...
class MultiIndex(object):
    ...
    @property
    def exponents(self):  # read only: must not be altered since transformation matrices etc. depends on this
    ...
    def __copy__(self):
        ...
        new_instance = self.__class__(self._exponents, self._lp_degree, self.poly_deg_dtype)
        self.copy_private_attributes(new_instance)
        return new_instance

    def __deepcopy__(self, memo):
        new_instance = self.__class__(deepcopy(self._exponents), deepcopy(self._lp_degree),
    ...
    def _new_instance_if_necessary(self, new_exponents: ARRAY) -> 'MultiIndex':
        ...
        if self._exponents_completed is not None:
            _exponents_completed = insert_lexicographically(self._exponents_completed, new_exponents)
            if _exponents_completed is not self._exponents_completed:  # some exponents have been added:
                # make complete again!
